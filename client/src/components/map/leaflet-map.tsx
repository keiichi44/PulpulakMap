...
import fountainIconSvg from "@/assets/fountain-icon.svg";
...
export default function LeafletMap({ fountains, userLocation, walkingRoute, nearestFountain }: LeafletMapProps) {
  ...
  useEffect(() => {
    ...
    // Custom CSS for user marker only
  ...
  useEffect(() => {
    ...
    fountains.forEach((fountain) => {
      ...
      const marker = L.marker([fountain.lat, fountain.lon], {
        icon: L.icon({
          iconUrl: fountainIconSvg,
          iconSize: isNearest ? [24, 24] : [20, 20],
          iconAnchor: isNearest ? [12, 12] : [10, 10],
          popupAnchor: [0, isNearest ? -12 : -10],
          className: isNearest ? "fountain-marker-nearest" : "fountain-marker-normal",
        }),
      });
